...
class Participant(User):
	...
	def get_insertion_string(self):
		return "'%s', '%s', '%s', '%s', '%s'" % (self._username, self._email, self._first_name, self._last_name, "PARTICIPANT")

class Biobanker(User):

	def get_insertion_string(self):
		return "'%s', '%s', '%s', '%s', '%s'" % (self._username, self._email, self._first_name, self._last_name, "BIOBANKER")

class Researcher(User):

	def get_insertion_string(self):
		return "'%s', '%s', '%s', '%s', '%s'" % (self._username, self._email, self._first_name, self._last_name, "RESEARCHER")
...
class SchemaTest(unittest.TestCase):
	...
	def test_users(self):
		...
		alice = Participant("al", "alice@uni.com", "Alice", "Jones")
		john = Participant("jo", "john@uni.com", "John", "Brown")
		biobanker = Biobanker("biobanker", "biobanker@uni.com", "No", "Name")
		scientist = Researcher("re", "scientist@uni.com", "Robot", "Man")
		...
		self.assert_fail_sql("""INSERT INTO users (
			...
			VALUES ('t_id', 't_email', 't_name', 't_surname', 'ADMIN');""", "DataError")
		...
		"""
		Insert a administrator into the database
		"""
		self._cursor.execute("""INSERT INTO users (
			user_id, email, first_name, last_name, role)
		...
		"""
		Remove a administrator from the users table
		"""
		self._cursor.execute("""DELETE FROM users
		...
		"""
		Remove a administrator from the biobankers table
		"""
...
def create_testing_environment():
	...
	with open(os.path.join(home, ".pgpass"), "r") as f:
		...
		schema.create_schema(TEST_DATABASE)
